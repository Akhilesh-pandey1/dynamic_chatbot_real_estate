...
def parse_intention_response(xml_response: str, state: dict) -> dict:
    response_match = re.search(r'<response>(.*?)</response>', xml_response, re.DOTALL)
    response = response_match.group(1) if response_match else None
    
    greeting_match = re.search(r'<greeting>(.*?)</greeting>', xml_response, re.DOTALL)
    greeting = greeting_match.group(1).lower() == 'true' if greeting_match else False
    
    standalone_match = re.search(r'<standalone>(.*?)</standalone>', xml_response, re.DOTALL)
