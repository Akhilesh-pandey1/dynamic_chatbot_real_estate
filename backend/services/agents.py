...
def create_user_intention_node(state):
    ...
    prompt_template = PromptTemplate(template=prompt, input_variables=["chat_history", "current_question"])
...
def create_rag_node(state):
    llm = load_model()
    relevant_chunks = get_relevant_chunks(state["username"], state["current_question"])
    ...
    prompt_template = PromptTemplate(template=prompt, input_variables=["context", "chat_history", "current_question"])
